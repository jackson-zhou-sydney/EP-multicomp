...
I.r.3 <- function(kappa, m, V, eta, mult, maxEval, tol) {
...
ep.approx <- function(X, y, mu.tau, sigma.2.tau, 
                      kappa.1, kappa.2, eta, alpha, lambda.init, 
  ...
  for (iteration in 1:max.passes) {
    ...
    for (i in sample(1:(n + 2*p))) {
      ...
      if (i <= n) {
        ...
        I.r.res <- tryCatch(I.r.2(kappa.1, m, V, eta, mult = 5, maxEval = 0, tol = 0.0001), error = err)
      } else {
        I.r.res <- tryCatch(I.r.3(kappa.2, m, V, eta, mult = 5, maxEval = 0, tol = 0.0001), error = err)
      }
