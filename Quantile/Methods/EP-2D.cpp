...
double rho(double x, double tau) {
  // Quantile loss function
  return 0.5*(abs(x) + (2.0*tau - 1)*x);
}

tuple <vec, mat> h_mom_2d(double y, double tau, vec mu, mat Sigma, double eta, int n_grid) {
  ...
  double adjust = 0.5*dot(mu, Q*mu) - dot(mu, r) + eta*(mu(1) + rho(y - mu(0), tau)/exp(mu(1)));
  ...
  for (int i = 0; i < n_grid; ++i) {
    for (int j = 0; j < n_grid; ++j) {
      ...
      Z_cube(0, i, j) = exp(-0.5*dot(xy, Q*xy) + dot(xy, r) - eta*(y + rho(y_data - x, tau)/exp(y)) + adjust);
...
// [[Rcpp::export]]
List ep_2d(mat X, vec y, mat Sigma_theta, vec mu_theta,
  ...
  for (int pass = 0; pass < max_passes; ++pass) {
    ...
    #pragma omp parallel
    {
      ...
      for (int k = 0; k < n; ++k) {
        ...
        tuple <vec, mat> h_mom_res = h_mom_2d(y(k), tau, mu_m_star, Sigma_m_star, eta, n_grid);
