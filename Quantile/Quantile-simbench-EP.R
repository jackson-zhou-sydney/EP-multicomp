...
# Simulations/benchmarks for quantile regression (EP)
...
for (type.iter in 1:num.each.type) {
  ...
  save(results.df, results.samples, file = paste0("Quantile/Quantile-results/Sim-", type.iter, "-res-EP.RData"))
}
...
for (type.iter in 1:num.each.type) {
  ...
  save(results.df, results.samples, file = paste0("Quantile/Quantile-results/Bench-", type.iter, "-res-EP.RData"))
}
