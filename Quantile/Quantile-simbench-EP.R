...
for (type.iter in 1:num.each.type) {
  ...
  save(results.df, file = paste0("Quantile/Quantile-results/Sim-", type.iter, "-res-EP.RData"))
}
...
for (type.iter in 1:num.each.type) {
  ...
  save(results.df, file = paste0("Quantile/Quantile-results/Bench-", type.iter, "-res-EP.RData"))
}
