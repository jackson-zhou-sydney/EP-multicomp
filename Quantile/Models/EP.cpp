...
tuple <vec, mat> h_mom(double y, double tau, vec mu, mat Sigma, double eta, int n_grid) {
  ...
  double adjust = -eta*(2.0*mu_2 + (2.0*tau - 1.0)*y/exp(mu_2));
  ...
  for (int i = 0; i < n_grid; ++i) {
    ...
    double c_l = Q_11*pow(mu_1, 2.0) + 2.0*Q_12*mu_1*(mu_2 - x) + Q_22*pow(x - mu_2, 2.0) + eta*(2.0*x + (2.0*tau)*y/exp(x)) + adjust;
    double c_u = Q_11*pow(mu_1, 2.0) + 2.0*Q_12*mu_1*(mu_2 - x) + Q_22*pow(x - mu_2, 2.0) + eta*(2.0*x + (2.0*tau - 2.0)*y/exp(x)) + adjust;
  ...
  for (int pass = 0; pass < max_passes; ++pass) {
    ...
    #pragma omp parallel
    {
      ...
      for (int k = 0; k < n; ++k) {
        ...
        tuple <vec, mat> h_mom_res = h_mom(y(k), tau, mu_m_star, Sigma_m_star, eta, n_grid);
