...
# Big data example for quantile regression
...
if (method == "mcmc") {
  load(paste0("Quantile/Results/Big-MCMC-results-", str_pad(seed + 1, 2, pad = "0"), ".Rdata"), envir = temp.env)
  ...
  bench.lppd.df <- bench.lppd.df %>% add_row(seed = seed,
                                             bench = 4,
                                             method = "mcmc",
                                             lppd = lppd(X.test, y.test, tau, tail.mcmc.samples))
} else if (method == "mcmc-s") {
  load(paste0("Quantile/Results/Big-MCMC-results-", str_pad(seed + 2, 2, pad = "0"), ".Rdata"), envir = temp.env)
  ...
  bench.lppd.df <- bench.lppd.df %>% add_row(seed = seed,
                                             bench = 4,
                                             method = "mcmc-s",
                                             lppd = lppd(X.test, y.test, tau, tail.mcmc.s.samples))
} else if (method == "ep") {
...
save(bench.l1.df, bench.mmd.df, bench.lppd.df, bench.cov.norm.df, bench.time.df,
     file = paste0("Quantile/Results/Big-results-", toupper(method), "-", str_pad(seed, 2, pad = "0"), ".RData"))
