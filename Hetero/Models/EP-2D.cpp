...
tuple <vec, mat> h_mom_2d(double y, vec mu, mat Sigma, double eta, int n_grid) {
...
// [[Rcpp::export]]
List ep_2d(mat X_1, mat X_2, vec y, mat Sigma_theta, vec mu_theta,
           double eta, double alpha, mat Q_star_init, vec r_star_init,
  ...
  for (int k = 0; k < n; ++k) {
    ...
    mat A = zeros(p_1 + p_2, 2);
    A.submat(0, 0, p_1 - 1, 0) = X_1.row(k).t();
    A.submat(p_1, 1, p_1 + p_2 - 1, 1) = X_2.row(k).t();
  ...
  for (int pass = 0; pass < max_passes; ++pass) {
    ...
    vec deltas_Q = zeros(n);
    vec deltas_r = zeros(n);
    ...
    #pragma omp parallel
    {
      mat Q_dot_local = zeros(p_1 + p_2, p_1 + p_2);
      vec r_dot_local = zeros(p_1 + p_2);
      ...
      for (int k = 0; k < n; ++k) {
        ...
        tuple <vec, mat> h_mom_res = h_mom_2d(y(k), mu_m_star, Sigma_m_star, eta, n_grid);
