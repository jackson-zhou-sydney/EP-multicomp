...
# Simulations for heteroscedastic linear regression
  ...
  for (iteration in 1:num.sim.iter) {
    ...
    if (method == "mcmc-g") {
      ...
      save(tail.mcmc.g.samples, mcmc.g.mu, mcmc.g.Sigma, grid.points, mcmc.g.values, 
           file = paste0("Hetero/Results/Simulations-results-MCMC-G-", type.iter, "-", str_pad(iteration, 2, pad = "0"), "-", str_pad(seed, 2, pad = "0"), ".RData"))
    } else if (method == "mcmc") {
      ...
      load(paste0("Hetero/Results/Simulations-results-MCMC-G-", type.iter, "-", str_pad(iteration, 2, pad = "0"), "-", str_pad(seed, 2, pad = "0"), ".RData"))
      rstan::mcmc.rstan <- stan_model("Hetero/Methods/MCMC.stan")
...
if (method != "mcmc-g") {
  save(sim.l1.df, sim.mmd.df, sim.lppd.df, sim.cov.norm.df, sim.time.df, sim.r.hat.df,
       file = paste0("Hetero/Results/Simulations-results-", toupper(method), "-", str_pad(seed, 2, pad = "0"), ".RData"))
}
