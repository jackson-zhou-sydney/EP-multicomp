...
# MCMC for heteroscedastic linear regression simulations
...
for (type.iter in 1:num.sim) {
  ...
  for (iteration in 1:num.sim.iter) {
    ...
    save(mcmc.g.mu, mcmc.g.Sigma, tail.mcmc.g.samples, grid.points, mcmc.g.values,
         ...
         file = paste0("Hetero/Results/Simulations-MCMC-results-", type.iter, "-", str_pad(iteration, 2, pad = "0"), "-", str_pad(seed, 2, pad = "0"), ".Rdata"))
  }
}
