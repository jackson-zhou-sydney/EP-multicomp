...
laplace.approx <- function(X, y, mu.beta, Sigma.beta, lambda.init, maxit) {
  ...
  start <- if (p > 2) as.vector(coef(glmnet(X[, -1], y, family = binomial(link = "probit"), alpha = 0, lambda = lambda.init))) else
                      as.vector(coef(glm(y ~ X[, -1], family = binomial(link = "probit"))))  
