...
TGI.lower.0 <- function(x) {
  # Truncated Gaussian integral (lower, 0th raw moment)
  a <- x[1]; b <- x[2]; c <- x[3]
  exp(log(sqrt(2*pi/a)) - 0.5*(c - b^2/(4*a)) + pnorm(0, -b/(2*a), sqrt(1/a), log.p = T))
}

TGI.lower.1 <- function(x) {
  # Truncated Gaussian integral (lower, 1st raw moment)
  a <- x[1]; b <- x[2]; c <- x[3]
  sqrt(2*pi/a)*(-(b/(2*a))*exp(-0.5*(c - b^2/(4*a)) + pnorm(b/(2*sqrt(a)), log.p = T)) - (1/sqrt(a))*exp(-0.5*(c - b^2/(4*a)) + dnorm(b/(2*sqrt(a)), log = T)))
}

TGI.lower.2 <- function(x) {
  # Truncated Gaussian integral (lower, 2nd raw moment)
  a <- x[1]; b <- x[2]; c <- x[3]
  sqrt(2*pi/a)*((b^2/(4*a) + 1)*exp(-0.5*(c - b^2/(4*a)) + pnorm(b/(2*sqrt(a)), log.p = T)) + (b/(2*sqrt(a)))*exp(-0.5*(c - b^2/(4*a)) + dnorm(b/(2*sqrt(a)), log = T)))/a
}

TGI.upper.0 <- function(x) {
  # Truncated Gaussian integral (upper, 0th raw moment)
  a <- x[1]; b <- x[2]; c <- x[3]
  exp(log(sqrt(2*pi/a)) - 0.5*(c - b^2/(4*a)) + pnorm(0, -b/(2*a), sqrt(1/a), lower.tail = F, log.p = T))
}

TGI.upper.1 <- function(x) {
  # Truncated Gaussian integral (upper, 1st raw moment)
  a <- x[1]; b <- x[2]; c <- x[3]
  sqrt(2*pi/a)*(-(b/(2*a))*exp(-0.5*(c - b^2/(4*a)) + pnorm(-b/(2*sqrt(a)), log.p = T)) + (1/sqrt(a))*exp(-0.5*(c - b^2/(4*a)) + dnorm(b/(2*sqrt(a)), log = T)))
}

TGI.upper.2 <- function(x) {
