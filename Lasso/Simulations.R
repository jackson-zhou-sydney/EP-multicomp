  ...
  for (iteration in 1:num.sim.iter) {
    ...
    if (method == "mcmc-g") {
      ...
      save(tail.mcmc.g.samples, mcmc.g.mu, mcmc.g.Sigma, mcmc.g.values, 
           file = paste0("Lasso/Results/Simulations-results-MCMC-G-", type.iter, "-", str_pad(iteration, 2, pad = "0"), "-", str_pad(seed, 2, pad = "0"), ".RData"))
    } else if (method == "mcmc") {
...
if (method != "mcmc-g") {
  save(sim.l1.df, sim.mmd.df, sim.lppd.df, sim.cov.norm.df, sim.time.df, sim.r.hat.df,
       file = paste0("Lasso/Results/Simulations-results-", toupper(method), "-", str_pad(seed, 2, pad = "0"), ".RData"))
}
